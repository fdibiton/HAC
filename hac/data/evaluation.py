...
class CocoCaptions(Dataset):
    ...
    def __init__(self, root: str | Path, split: str, transform: Callable | None = None, subsample_ratio: float = None):
        """
        Args:
        ...
        if self.subsample_ratio is not None:
            num_samples = int(len(self.samples) * self.subsample_ratio)
            # uniformly sample a subset of samples
            step = max(1, len(self.samples) // num_samples)
            self.samples = self.samples[::step][:num_samples]
